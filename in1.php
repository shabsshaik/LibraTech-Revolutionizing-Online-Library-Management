...
<CENTER><h1 align="center" style="margin: 20px 0;"><u>UPLOAD FACULTY RECORD</h1></U>
    ...
    if(in_array($file_ext,$allowed_ext))
    {
		...
		foreach ($excel->rows($sheet) as $key => $row) {
	       ...
	       $branch= $row['1'];
			...
			$checkStudent="SELECT  * FROM staff WHERE staff_id='$branch' order by staff_id";
            $checkStudent_result=mysqli_query($con,$checkStudent);
            if(mysqli_num_rows($checkStudent_result)>0)
            {
                $in_query="UPDATE `staff` SET `depart`='$id',`staff_id`='$branch',`names`='$regnum' where staff_id='$branch'";
            ...
            {
                $in_query="INSERT INTO  staff(depart,staff_id,names) VALUES ('$id','$branch','$regnum')";
                $in_result=mysqli_query($con,$in_query);
               $msg=1;
            }
          }
...
<form action="importfile1.php" method="POST">
<table border=1 >
  <TR>
  <th>
  <center>
    ...
    <th><center>
    EMPLOYEE ID</center></th>

    <th><center>
    DEPARTMENT</center></th>
